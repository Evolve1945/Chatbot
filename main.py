...
def tf(text : str) -> dict:
  ...
  frequency = dict(sorted(frequency.items(), key=lambda item: item[0]))       # Sort the frequency dictionary by word in ascending order
...
def idf(folder : str) -> dict:
  ...
  for files in os.listdir(folder):                                                              # Iterate over each file in the specified folder
    with open(os.path.join(folder, files), 'r', encoding = 'utf-8') as file:                    # Open the file in read mode with UTF-8 encoding
      ...
      words = file.read().split()                                                               # Read the contents of the file and split it into words
